...
pub trait GcTypeLayouts {
    ...
    ///
    /// Returns `None` if the type is not managed by the GC. Currently, this
    /// includes function types and continuation types. Note that continuation
    /// types are planned to be GC-managed in the future (see #10248).
    fn gc_layout(&self, ty: &WasmCompositeType) -> Option<GcLayout> {
        ...
        match &ty.inner {
            ...
            WasmCompositeInnerType::Cont(_) => None, // TODO(#10248): continuation objects will be GC-managed in the future
